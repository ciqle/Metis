...
- 🎯 Entry points to execute your program using `python -m <metis>` or `$ metis` with basic CLI argument parsing.
...
---
# metis Flask Application

Awesome metis created by ciqle
...
```bash
git clone https://github.com/ciqle/Metis metis
cd metis
make install
```
...
```bash
metis create-db   # run once
metis populate-db  # run once (optional)
metis add-user -u admin -p 1234  # ads a user
metis run
```
